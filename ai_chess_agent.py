...
if "minimax_api_key" not in st.session_state:
    st.session_state.minimax_api_key = DEFAULT_API_KEY or None
if "minimax_base_url" not in st.session_state:
    st.session_state.minimax_base_url = DEFAULT_BASE_URL
if "minimax_model" not in st.session_state:
    st.session_state.minimax_model = DEFAULT_MODEL
...
minimax_api_key = st.sidebar.text_input(
    "MiniMax API key:",
    type="password",
    value=st.session_state.minimax_api_key or "",
    help="Loaded from MINIMAX_API_KEY in .env if set.",
)
if minimax_api_key:
    st.session_state.minimax_api_key = minimax_api_key
    st.sidebar.success("API key saved!")

st.sidebar.caption(f"Model: `{st.session_state.minimax_model}`")
st.sidebar.caption(f"Base URL: `{st.session_state.minimax_base_url}`")
...
if st.session_state.minimax_api_key:
    try:
        ...
        st.info("""
This chess game is played between two AG2 AI agents powered by **MiniMax-M3**:
- **Agent White**: MiniMax-M3 chess player controlling white pieces
- **Agent Black**: MiniMax-M3 chess player controlling black pieces
...
""")
        ...
        if st.button("Start Game"):
            ...
            st.success(
                "You can view the interaction between the agents in the terminal output, after the turns "
                "between agents end, you get view all the chess board moves displayed below!"
            )
        ...
        st.error(f"An error occurred: {e}. Please check your MiniMax API key and try again.")

else:
    st.warning("Set MINIMAX_API_KEY in .env or enter your MiniMax API key in the sidebar to start.")
